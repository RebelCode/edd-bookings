...
class Assets
{
    ...
    public function hook()
    {
        ...
        $this->getPlugin()->getHookManager()
                ->addAction(static::HOOK_FRONTEND, $this, 'enqueuePublicStyles')
                ->addAction(static::HOOK_FRONTEND, $this, 'enqueuePublicScripts')
                ->addAction(static::HOOK_ADMIN, $this, 'enqueueAdminStyles')
                ->addAction(static::HOOK_ADMIN, $this, 'enqueueAdminScripts');
    }
     ...
     * @param string|null $media The media, on which this stylesheed should have effect. Default: 'all'.
     * @return Assets
     */
    protected function _registerStyle($handle, $url, $dependencies = null, $version = null, $media = null,
            $allowOverwrite = false)
    {
        return $this->register(self::ASSET_TYPE_STYLE,
                        array(
                        'handle'  => $handle,
                        'version' => $version,
                        'uri'     => $url,
     ...
     * @return Assets
     */
    protected function _registerScript($handle, $url, $dependencies = null, $version = null, $inFooter = false,
            $allowOverwrite = false)
    {
        return $this->register(self::ASSET_TYPE_SCRIPT,
                        array(
                        'handle'    => $handle,
                        'version'   => $version,
                        'uri'       => $url,
     ...
     *                    <br/>* The 'handle' value will be prefixed, unless overridden/
     *                    <br/>* The 'uri' value will be made absolute, unless already absolute.
     * @param array|null $dependencies Handles of asset that this asset depends on, if any.
     * @throws Exception If invalid 'type' value, or 'handle' or 'uri' not supplied.
     */
    public function register($assetType, $data, $dependencies = null, $allowOverwrite = false)
    {
        $type = trim($assetType);
        if (!static::hasAssetType($type)) {
            throw new Exception(
            sprintf('Could not register asset of type "%1$s": Type is invalid, please use one of [%2$s]', $type,
        ...
        $dependenciesArray = (array) $dependencies;
        // Must provide handle
        if (!isset($data['handle'])) {
            throw $this->exception(array('Could not register asset of type "%1$s": Handle must be provided',
                    $type));
        }
        // Must provide uri
        if (!isset($data['uri'])) {
            throw $this->exception(array('Could not register asset "%2$s" of type "%1$s": URI must be provided',
                    $type, $data['handle']));
        }
        ...
        return $this->_register($type, $data, $dependenciesArray, $allowOverwrite);
    }
    ...
    protected function _registerAsset($asset)
    {
        ...
        switch ($type) {
            case static::ASSET_TYPE_SCRIPT:
                wp_register_script(
                        (string) $asset['handle'], (string) $asset['uri'], (array) $asset['dependencies'],
                        (string) $asset['version'], (bool) $asset['in_footer']
                );
                return true;
                break;
            ...
            case static::ASSET_TYPE_STYLE:
                wp_register_style(
                        (string) $asset['handle'], (string) $asset['uri'], (array) $asset['dependencies'],
                        (string) $asset['version'], (string) $asset['media']
                );
                return true;
                break;
        }
     ...
     * @throws Exception If no handle provided, or no asset registered for handle.
     */
    protected function _enqueueAsset($asset)
    {
        ...
        if (!($asset = $this->getAsset($asset))) {
            throw new Exception(sprintf('Could not enqueue asset "%1$s": No asset registered with that handle', $asset));
        }
        $type = $asset['type'];
     ...
     * @param string|null $media The CSS media type. If null, 'all' will be used.
     ...
     */
    public function registerStyle($handle, $uri, $dependencies = null, $version = null, $media = null,
            $allowOverwrite = false)
    {
     ...
     * @throws Exception If the handle isn't registered, the handle type is not registered or is not of a style.
     */
    public function enqueueStyle($handle)
    {
        ...
        if (!($asset = $this->getAsset($handle))) {
            throw new Exception(sprintf('Could not enqueue script "%1$s": Register the script first', $handle));
        }
        if (!$this->hasAssetType($asset['type'])) {
            throw new Exception(sprintf('Could not enqueue style "%1$s": "%2$s" is not a registered asset type',
                    $handle, $asset['type']));
        }
        if ($asset['type'] !== static::ASSET_TYPE_STYLE) {
            throw new Exception(sprintf('Could not enqueue style "%1$s": "%2$s" is not a style type', $handle,
                    $asset['type']));
        }
     ...
     * @return string|null The asset handle if regisered; otherwise null.
     */
    public function registerScript($handle, $uri, $dependencies = null, $version = null, $inFooter = false,
            $allowOverwrite = false)
    {
     ...
     * @return AssetsAbstract This instance.
     * @throws Exception If the handle isn't registered, the handle type is not registered or is not of a script.
     */
    public function enqueueScript($handle)
    {
        ...
        if (!$this->hasAssetType($asset['type'])) {
            throw new Exception(sprintf('Could not enqueue script "%1$s": "%2$s" is not a registered asset type',
                    $handle, $asset['type']));
        }
        if ($asset['type'] !== static::ASSET_TYPE_SCRIPT) {
            throw new Exception(sprintf('Could not enqueue style "%1$s": "%2$s" is not a script type', $handle,
                    $asset['type']));
        }
        $this->_enqueueAsset($handle);
    ...
    public function getHandlePrefix($handle = null)
    {
        ...
        return is_null($handle) ? $prefix : static::stringHadPrefix($handle) ? $handle : "{$prefix}{$handle}";
    }
